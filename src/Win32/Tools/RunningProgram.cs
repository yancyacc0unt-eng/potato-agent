...
/// <param name="Hwnd">那个已有窗口的句柄；<see cref="IntPtr.Zero"/> 表示同名进程在跑但眼下没有可见窗口。</param>
...
internal static class RunningProgram
{
    ...
    private const int MinTitleStemLength = 3;
    ...
    internal static RunningInstance? Find(string? path)
    {
        ...
        foreach (var hwnd in Desktop.VisibleWindows())
        {
            ...
            if (processName.Length > 0 && string.Equals(processName, stem, StringComparison.OrdinalIgnoreCase))
            {
                return new RunningInstance(
                    hwnd, pid, processName, title,
                    $"the running {processName} process owns this window");
            }

            if (byTitle is null &&
                stem.Length >= MinTitleStemLength &&
                title.Contains(stem, StringComparison.OrdinalIgnoreCase))
            {
                byTitle = new RunningInstance(
                    hwnd, pid, processName.Length > 0 ? processName : "unknown", title,
                    $"its window title contains \"{stem}\"");
            }
        }

        // 标题命中排在进程名命中之后：标题只能说明"看着像"，进程名才是硬的。
        if (byTitle is not null)
        {
            return byTitle;
        }

        // 兜底：同名进程在跑，但一个可见窗口都没有（后台常驻、还没画出窗口、或者窗口被隐藏了）。
    ...
    /// <summary>目标的可执行名/文件名去掉扩展名后的词（"notepad.exe" → "notepad"）。</summary>
