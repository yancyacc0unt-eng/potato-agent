...
//   六、思考模式    ：reasoning_content 是独立事件、不混正文；写进 assistant 消息并在后续
//                     每个请求里原样回传（带 tools 时不回传服务端会 400）；没给时不许造空字段
//   七、模型列表    ：GET /v1/models 的端点补全、正常列表（排序去重 + 请求形状）、401、
//                     服务端没实现这个端点、空列表，以及模型名缓存（knownModels）的落盘
//
// ⚠ 安全前提（本工程所有路径都遵守）：
